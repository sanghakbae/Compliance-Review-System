...
- [~] **Phase 5** 클라이언트 컷오버 — **진행 중**
  ...
  - [ ] **`documentService.ts`(2,892줄)** — 함수별 전환 지침: [PHASE5-documentService-cutover.md](PHASE5-documentService-cutover.md). `.from()` 61곳→Firestore, storage 4곳→Worker R2, 백엔드 작업→Worker `/functions`. *앱 실행 검증 필요.*
